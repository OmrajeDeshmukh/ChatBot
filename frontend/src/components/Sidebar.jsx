        ...
        <div className="max-h-[500px] overflow-y-auto mb-20 md:mb-0 thin-scrollbar">
          {Array.isArray(chats) && chats.length > 0 ? (
            chats.map((e) => (
              <div
                key={e._id}
                className="w-full py-2 px-2 bg-gray-700 hover:bg-gray-600 rounded mt-2 flex justify-between items-center"
              >
                <button
                  onClick={() => clickEvent(e._id)}
                  className="text-left flex-grow"
                >
                  {e.latestMessage?.slice(0, 38) || "No message"}...
                </button>

                <button
                  className="bg-red-600 text-white text-xl px-3 py-2 rounded-md hover:bg-red-700 ml-2"
                  onClick={(event) => {
